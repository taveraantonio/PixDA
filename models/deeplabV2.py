...
class ResNetMulti(nn.Module):
    ...
    def forward(self, x, use_classifier=True, get_features=False):
        x1 = None
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x2 = self.layer4(x)         # produce intermediate features
...
def deeplabv2_resnet(num_classes=19, multi_level=True):
